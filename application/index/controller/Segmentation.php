...
class Segmentation extends Controller
{
    ...
    public function cut()
    {
        ...
        $reg_list = $this->getTop($content, 200);

        $value = [];
        foreach ($reg_list as $word => $weight) {
            $value[] = [
                'name' => $word,
                'value' => round($weight * 10000),
                'itemStyle' => $this->createRandomItemStyle()
            ];
        }

        $image_file = !$image ? $this->getImage() : $this->getBase64($image->getInfo()['tmp_name']);
    ...
    public function createRandomItemStyle()
    {
